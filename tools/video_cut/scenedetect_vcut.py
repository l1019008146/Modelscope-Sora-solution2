...
def parse_args():
    ...
    parser.add_argument("--input_csv", type=str, required=True, help="The path to the CSV file containing video paths.")
    parser.add_argument(
        '--min_seconds', 
        type = int,
        default = 3,
...
def main():
    # Args
    args = parse_args()
    video_df = pd.read_csv(args.input_csv)
    video_path_list = video_df['path'].tolist()
    ...
    with Pool(processes=num_processes) as pool:
        with tqdm(total=len(video_path_list)) as progress_bar:
